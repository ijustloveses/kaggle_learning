...
class RandomLeaderClassifier(object):
  ...
  def project(self, X_train):
    ...
    # 从 X_train 中抽样 (self.nr_projections, self.size_projections) 个 feature，注意 shape[1] 是 feature，不是 样本
    self.random_indexes = np.random.randint(0, high=X_train.shape[1], size=(self.nr_projections, self.size_projections))
    # 初始化 (self.nr_projections, self.size_projections) 个 weight
    self.random_weights = np.random.rand(self.nr_projections,self.size_projections)
    # 遍历 X_train
    for e, x in enumerate(X_train):
      if e == 0:
        # 首次遍历时，生成 thresholds，方法是 X_train[0] 对应 self.random_indexes 那些feature的值，乘以 self.random_weights
        # 然后，再按 self.size_projections 方向求和，得到一个 self.nr_projections 个分量的向量，最后转为矩阵 1 X self.nr_projections
        thresholds = np.sum(x[self.random_indexes] * self.random_weights, axis=1).reshape((1,self.nr_projections))
      else:
        # 随后采样并拼接 threshold 后续的部分
